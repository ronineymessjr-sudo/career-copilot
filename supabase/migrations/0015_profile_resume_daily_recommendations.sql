...
alter table public.profiles alter column graduation_year drop not null;
alter table public.profiles alter column graduation_year drop default;
...
alter table public.resume_versions drop constraint if exists resume_versions_source_type_check;
alter table public.resume_versions add constraint resume_versions_source_type_check
...
create index if not exists resume_versions_profile_status_updated_idx
  on public.resume_versions(profile_id, status, updated_at desc);

create table if not exists public.daily_recommendation_preferences (
...
alter table public.daily_recommendation_preferences enable row level security;
alter table public.daily_recommendations enable row level security;

drop policy if exists daily_recommendation_preferences_owner_all on public.daily_recommendation_preferences;
create policy daily_recommendation_preferences_owner_all on public.daily_recommendation_preferences
  for all to authenticated
  using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);

drop policy if exists daily_recommendations_owner_select on public.daily_recommendations;
create policy daily_recommendations_owner_select on public.daily_recommendations
  for select to authenticated
  using ((select auth.uid()) = user_id);

grant select, insert, update, delete on public.daily_recommendation_preferences to authenticated;
...
-- public.resume_versions; uploaded PDF/DOC/DOCX/TXT bytes are stored here.
