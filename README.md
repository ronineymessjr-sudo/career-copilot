...
[![WorkBuddy Expert](https://img.shields.io/badge/WorkBuddy-Expert-purple)](https://codebuddy.cn)
...
- 多入口接入：Web、CLI、WorkBuddy 专家、MCP 工具
...
| 入口 | 适合场景 | 使用方式 |
|------|---------|---------|
| **Web App** | 日常求职操作、可视化看板 | [在线使用](https://career-copilot-v2.photomagic.workers.dev) |
| **CLI** | 命令行快速搜索、生成简历 | `npm run cli -- search`（详见下方） |
...
Career Copilot 采用**统一引擎 + 多入口**设计：

- **Web 工作台**：可视化操作面板
- **CLI 工具**：本地命令行，无需部署、无需云端账号
- **WorkBuddy 专家**：对话式引导
- **MCP 工具**：让任意 Agent 调用求职能力
...
Web 端、CLI、WorkBuddy 专家、MCP 端均内置反馈入口，随时随地可以提交。
