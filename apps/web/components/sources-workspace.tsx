...
export function SourcesWorkspace() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setSources(result.sources ?? []); setRuns(result.runs ?? []); setMessage("");
      ...
      <section className="platform-panel">
        ...
        <div className="platform-source-list">
          {sources.length ? sources.map((source) => <article key={source.id}>
            ...
            <div className="platform-source-health"><strong className={`state-${source.last_status}`}>{source.last_status === "success" ? "正常" : source.last_status === "failed" ? "失败" : source.last_status === "partial" ? "部分成功" : "尚未运行"}</strong><span>{source.last_checked_at ? new Date(source.last_checked_at).toLocaleString("zh-CN") : "等待首次发现"}</span><small>{source.last_error || `新增 ${source.last_result?.imported ?? 0} · 更新 ${source.last_result?.updated ?? 0}`}</small></div>
