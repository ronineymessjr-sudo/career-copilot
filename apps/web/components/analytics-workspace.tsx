...
export function AnalyticsWorkspace({ compact = false }: { compact?: boolean }) {
  ...
  return <section className={compact ? "analytics-workspace compact" : "control-panel analytics-workspace"}>
    ...
    {!compact ? <nav className="platform-priority-rail" aria-label="数据洞察优先级">
      <a className="platform-priority-item p0" href="#analytics-funnel"><span>P0 · 现在看</span><strong>{metrics.submitted ?? 0} 个已确认投递</strong><small>先看回复、面试和 Offer 转化</small></a>
      <a className="platform-priority-item p1" href="#analytics-health"><span>P1 · 接着处理</span><strong>{data?.observability?.failure_count ?? 0} 个控制面失败</strong><small>有异常时先处理健康问题</small></a>
      <a className="platform-priority-item p2" href="#analytics-review"><span>P2 · 需要时看</span><strong>周度复盘与细分</strong><small>需要调整策略时再展开</small></a>
    </nav> : null}
    ...
    <div className="analytics-primary-grid">
      <section className="funnel-panel" id="analytics-funnel"><div className="panel-title"><BarChart3 size={16}/><div><strong>申请漏斗</strong><span>转化率以上一阶段为分母</span></div></div><div className="funnel-list">{funnel.map((item: Row) => <div key={item.stage}><div><strong>{STAGE_LABELS[item.stage] ?? item.stage}</strong><span>{item.count} · {item.conversion_from_previous}%</span></div><div className="funnel-track"><i style={{ width: `${Math.max(3, Number(item.count) / maxCount * 100)}%` }}/></div></div>)}</div></section>
      <section className="health-panel" id="analytics-health"><div className="panel-title"><Activity size={16}/><div><strong>生产健康</strong><span>最近任务和控制面事件</span></div></div><div className="health-row"><span>来源异常（最近 10 次）</span><strong>{data?.source_health?.failures_last_10 ?? 0}</strong></div><div className="health-row"><span>控制面失败</span><strong>{data?.observability?.failure_count ?? 0}</strong></div><div className="health-row"><span>待补技能</span><strong>{data?.open_skill_gaps?.length ?? 0}</strong></div>{data?.observability?.failure_count ? <div className="blocker-box"><TriangleAlert size={15}/><div><strong>需要检查</strong><p>{data.observability.recent_failures.map((item: Row) => item.event_name).join(" · ")}</p></div></div> : <div className="ready-box"><CheckCircle2 size={15}/>当前未记录新的控制面失败。</div>}</section>
    </div>
    {!compact ? <><section className="analytics-platform-grid">
      <section className="funnel-panel"><div className="panel-title"><Target size={16}/><div><strong>完整产品漏斗</strong><span>从岗位发现到 Offer，区分推荐、收藏、材料和实际投递</span></div></div><div className="funnel-list">{(data?.product_funnel ?? []).map((item: Row) => <div key={item.stage}><div><strong>{STAGE_LABELS[item.stage] ?? item.stage}</strong><span>{item.count} · {item.conversion_from_previous}%</span></div><div className="funnel-track"><i style={{ width: `${Math.max(3, Number(item.count) / Math.max(1, Number(data?.product_funnel?.[0]?.count ?? 1)) * 100)}%` }}/></div></div>)}</div></section>
      <section className="analytics-table source-quality-table"><h3>岗位来源质量</h3><div className="analytics-table-head"><span>来源</span><span>开放岗位</span><span>过期/关闭</span><span>健康分</span></div>{(data?.source_quality ?? []).length ? (data?.source_quality ?? []).slice(0, 10).map((row: Row) => <div className="analytics-table-row" key={row.source_id}><strong>{row.label}</strong><span>{row.open}</span><span>{row.stale + row.closed}</span><span>{row.health_score}</span></div>) : <p className="muted-copy">还没有来源质量数据。</p>}</section>
    </section><div className="analytics-breakdowns"><RateTable title="渠道表现" rows={data?.analytics?.breakdowns?.channel ?? []}/><RateTable title="地区表现" rows={data?.analytics?.breakdowns?.location ?? []}/><RateTable title="简历版本" rows={data?.analytics?.breakdowns?.resume ?? []}/></div>
      <div className="review-grid" id="analytics-review"><section className="weekly-review"><div className="panel-title"><CalendarRange size={16}/><div><strong>周度复盘</strong><span>只生成分析与建议，不执行投递或沟通动作</span></div><button className="ghost-button" disabled={busy} onClick={() => void generateReview()}>重新生成</button></div>{reviews[0] ? <><div className="weekly-metrics">{Object.entries(reviews[0].summary?.metrics ?? {}).slice(0, 5).map(([key, value]) => <span key={key}>{key}: <strong>{String(value)}</strong></span>)}</div><h4>下一步</h4><ol>{(reviews[0].summary?.next_actions ?? []).map((item: string) => <li key={item}>{item}</li>)}</ol></> : <p className="muted-copy">尚未生成周报。</p>}</section>
