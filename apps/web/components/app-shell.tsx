...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="platform-frame">
    ...
    <aside className="platform-sidebar">
      ...
      <div className="platform-sidebar-account"><span><UserRound size={16}/><small title={email || t("guestAccount")}>{email || t("guestAccount")}</small></span>{email ? <button type="button" onClick={() => void signOut()}><LogOut size={15}/>{t("signOut")}</button> : <Link className="platform-sidebar-signin" href={`/login?next=${encodeURIComponent(pathname || "/dashboard")}`}>{t("signInWorkspace")}</Link>}</div>
    </aside>
    <div className="platform-content">
      <header className="cc-mobile-nav" onKeyDown={(event) => { if (event.key === "Escape") { setMobileOpen(false); document.getElementById("workspace-menu-toggle")?.focus(); } }}>
        ...
        <nav id="workspace-mobile-links" aria-label="移动端工作区" hidden={!mobileOpen}>
          ...
          {email ? <button className="cc-button" type="button" onClick={() => void signOut()}><LogOut size={16}/>{t("signOut")}</button> : <Link className="cc-button" href={`/login?next=${encodeURIComponent(pathname || "/dashboard")}`}>{t("signInWorkspace")}</Link>}
        </nav>
      </header>
